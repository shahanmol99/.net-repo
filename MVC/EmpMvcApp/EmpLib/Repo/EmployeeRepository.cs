...
namespace EmpLib.Repo
{
    public class EmployeeRepository : IEmployeeRepository
    {
        ...
        public int GetDept(string deptName)
        {
            string queryString = "SELECT DEPTNO FROM DEPT WHERE DNAME = @deptName";
            var cmd = new SqlCommand(queryString, _sqlCon);
            SqlParameter deptNoParam = new SqlParameter("@deptName", deptName);
            ...
            {
                dno = Convert.ToInt32(dataReader["DEPTNO"]);
            }
